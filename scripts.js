...
var animationCanvas = document.getElementById('animationCanvas'),
    ...
    w = animationCanvas.width = window.innerWidth,
    h = animationCanvas.height = window.innerHeight,
    opts = {
        chars: '\1234567890ìqwertyuiopè+asdfghjklòàù<zxcvbnm,.-|!"£$%&/()=?^QWERTYUIOPé*ASDFGHJKLç°§>ZXCVBNM;:_[]@#€{}'.split(''),
        font: '15px monospace',
        charSize: 17,
        lineHeight: 17,
        hueSpeed: 1,
...
function loop() {
    ...
    for (var i = 0; i < num; ++i) {
        ...
        ctx.fillStyle = 'hsl(' + ((x + y) / sum * 360 + tick) + ',80%,50%)';
...
function resizeCanvas() {
    w = animationCanvas.width = logoCanvas.width = window.innerWidth;
    h = animationCanvas.height = logoCanvas.height = window.innerHeight;
...
var logo = new Image();
logo.src = 'https://lgbt-feed.pages.dev/logo.png'; // Update the path to your logo
logo.onload = drawLogo;

function drawLogo() {
    var maxWidth = w * 0.8; // Max width for the image
    var maxHeight = h * 0.8; // Max height for the image
    ...
    logoCtx.clearRect(0, 0, w, h); // Clear the canvas before drawing
    logoCtx.drawImage(logo, (w - width) / 2, (h - height) / 2, width, height); // Draw the logo centered
}